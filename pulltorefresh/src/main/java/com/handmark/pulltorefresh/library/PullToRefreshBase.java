...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {  /// 发生滚动，并且禁止当前view做任何滚动。
                    return true;
                }
                ...
                if (isReadyForPull()) {
                    ...
                    if (absDiff > mTouchSlop
                        ...
                        } else if (mMode.showFooterLoadingLayout() && diff <= -1f   ///发生的上拉刷新，并且已经发生，并且已经处于ready状态
                                && isReadyForPullEnd()) {
                            /// 开始发生了上拉行为，有了1像素的移动。
                            mLastMotionY = y;
                            mLastMotionX = x;
            ...
            case MotionEvent.ACTION_DOWN: {
                if (isReadyForPull()) { /// 可以查看一下这个方法，这是个临界方法
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    ///用户已经不跟listview发生交互了，我还没有想明白什么情况能到这。
                    setState(State.RESET);
    ...
    private void pullEvent() {
      /// 这个函数的细节里面做了什么。foot、header view 的隐藏和显示；foot header 拉的时候的调用；状态变迁；偏移量的变更
        ...
        switch (mCurrentMode) {
            case PULL_FROM_END:
                newScrollValue = Math.round(Math.max(initialMotionValue
                        - lastMotionValue, 0)
                        / FRICTION);    ///     ??? FRICTION 作用,猜测是用来减少偏移量的
